...
if(knitr::is_latex_output()) {
  ...
  knit_hooks$set(plot = function(x, options) { 
    x <- c("\\vspace{5pt}", "\\phantom{X}", 
           plot_default(x, options), 
           "\\vspace{2pt}", "\\phantom{X}")
  })
}
...
# Wasatch Standards
spectrometerColors <- c("#cbc392", "#090909", "#d53242", "#fd8e25", "#009e51", "#8f47ae", "#a32d90", "#bdbfbc", "#325b93", "#57a0c3", "#bdb427", "#00a5b5")
names(spectrometerColors) <- c("WP-1064", "WP-830",  "WP-785",  "WP-633",  "WP-532", "WP-405", "WP-248", "VIS-NIR", "VIS", "UV-VIS", "UV-VIS-NIR")
...
```{r}
getInfoFromWPSC <- function(WPSCtextInput, keyword, column = 2, regexFixed = FALSE) {
  # gets the info after a keyword from a single line in the CSV file
  
  infoLine <- grep(keyword, WPSCtextInput, fixed = regexFixed)
  infoText <- paste0(WPSCtextInput[infoLine], "\n")
  parseText <- unlist(read_csv(file=infoText, col_names=FALSE)[1, column])
  return(parseText)
}


getRowFromWPSC <- function(WPSCtextInput, keyword, regexFixed = FALSE) {
  # gets all the info after a keyword from a single line in the CSV file as a vector
  
  infoLine <- grep(keyword, WPSCtextInput, fixed = regexFixed)
  infoText <- paste0(WPSCtextInput[infoLine], "\n")
  parseText <- unlist(read_csv(file=infoText, col_names=FALSE)[1, -1])
  return(parseText)
}


getTableFromWPSC <- function(WPSCtextInput, keyword, previousKeyword = NULL) {
  # gets a table/matrix following the "keyword"
  # as the keyword is often generic (Results, Spectra), 
  # a more specific, preceding keyword can be provided
  # the end of the table is defined by an empty line
  
  startLine <- 0
  # if a pre-select keyword is given
  if (! is.null(previousKeyword)) {
    startLine <- grep(previousKeyword, WPSCtextInput, fixed = TRUE)[1]
  }
  # all lines with main keyword
  infoStartLines <- grep(keyword, WPSCtextInput, fixed = TRUE)
  # first one beyond the pre-select keyword
  infoStartLine <- (infoStartLines[infoStartLines > startLine])[1]
  
  # now find end line
  blankLine <- ""
  endLines <- which(nchar(WPSCtextInput) == 0)
  endLine <- (endLines[endLines > infoStartLine])[1]
  
  tableText <- WPSCtextInput[(infoStartLine+1):(endLine-1)]
  tableInfo <- read_csv(tableText)
...
```{r warning=FALSE, message=FALSE}
# import routines for old Enlighten format

oldEnlightenMetaData <- function(fileName, filePath = NULL) {
  ...
  metaDataHeader <- read_lines(fileName, n_max = maxLines, skip = 0)
  
  lastLine <- which(gsub(pattern = ",", replacement = "", x = metaDataHeader) == "") - 1
  
  # now each line is a pair of comma-separated values

  spectraHeader <- metaDataHeader[lastLine + 2]
  colLabels <- read_csv(gsub(",", "\n", spectraHeader), col_names = FALSE)
  nonSpectraLabel <- c("Pixel", "Wavelength", "Wavenumber")
  
  spectraIndex <- which(!(colLabels$X1 %in% nonSpectraLabel))
  numSpectra <- length(spectraIndex)
  ...
  # now turn list of lines into data frame
  metaData <- read_csv(metaDataTrimmed, col_names = FALSE)
  
  names(metaData) <- c("Parameter", "Value")
...
oldEnlightenSpectra <- function(fileName, filePath = NULL) {
    # repeat code to get columns that matter and start line
  ...
  emptyLine <- which(gsub(pattern = ",", replacement = "", x = metaDataHeader) == "")
  
  # read spectra
  spectraInput <- read.csv(fileName, header = TRUE, skip = emptyLine)

}


oldEnlightenGetSpectra <- function(spectraInput) {
    
    # pick columns with processed data
    nonSpectraLabel <- c("Pixel", "Wavelength", "Wavenumber")
  
    spectraIndex <- which(!(names(spectraInput) %in% nonSpectraLabel))
    numSpectra <- length(spectraIndex)
...
```{r fontImport, message=FALSE, warning=FALSE, cache=TRUE}
library(extrafont)
